...
export default class Linter {
  ...
  checkNGCharForKeywords(line, pos) {
    // NG文字を探す
    Array.prototype.forEach.call(line, function(c, i) {
      var m = c.match(/([<>=^])/)
      if (m) {
        this.addMessage('error', [[pos[0], i], [pos[0], i + 1]], `unexpected "` + m[0] + `"`)
      }
    }, this)
  }

  checkProposal(lines, relPos) {
    var pos = [relPos[0], relPos[1]]
    var s = ""
  ...
  checkKeywords(lines, relPos) {
    ...
    lines.forEach(function(line, i){
      this.checkZenkaku(line, pos)
      this.checkVoice(line, pos)
      this.checkNGCharForKeywords(line, pos)
      pos [0] += 1
    }, this)
  }

  checkUtterance(lines, relPos) {
    var pos = [relPos[0], relPos[1]]
    var s = ""

    this.checkKeywords([lines[0]], pos)
    pos[0] += 1
    this.checkProposal(lines.slice(1), pos)
  }

  checkLine(stack) {
    if (stack["type"] == "p") {
      this.checkProposal(stack["lines"], this.pos)
    } else if (stack["type"] == "u") {
      this.checkUtterance(stack["lines"], this.pos)
    } else if (stack["type"] == "c") {
      this.checkKeywords(stack["lines"], this.pos)
    }
